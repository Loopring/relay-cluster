...
---
## 如何部署中继
目前提供两种部署方式：[aws部署](deploy/deploy_index_cn.md)和[docker部署](relay_docker_cn.md)

分布式中继是一个完整的企业级分布式应用，虽然目前系统体量比较小，仅包含3个微服务，但同其他分布式应用一样，具有如下特点：
...
### 对比旧版
旧版中继虽能完整的支持钱包和DEX服务，但在架构上却存在单点故障，不仅在升级和扩展方面异常困难，还具有性能瓶颈和无法降级等常见系统问题。

为达到企业级应用的标准，向用户提供高可用高性能的钱包和DEX服务，旧版中继被拆分成包含中继、撮合和解析器三个微服务的分布式版本。
